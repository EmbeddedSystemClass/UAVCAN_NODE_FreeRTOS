...
# CRC equivalent to crc_crc32() in Ardupilot AP_Math/crc.cpp
...
''' Script assumes Python version of 3.4 or higher '''
assert sys.version_info >= (3, 4)

msg = "Applying app_descriptor to binary"
print(msg)

''' 40 A2 E4 F1 64 68 91 06 '''
descriptor = b'\x40\xa2\xe4\xf1\x64\x68\x91\x06'

''' img = open("C:/Users/geoma/Desktop/test/UAVCAN/UAVCAN_NODE_FreeRTOS/build/UAVCAN_NODE_FreeRTOS.bin", 'rb').read() '''
filepath = Path(__file__).parent / "../build/UAVCAN_NODE_FreeRTOS.bin"
print(filepath)
img = open(filepath, 'rb').read()

offset = img.find(descriptor)
if offset == -1:
    sys.exit("Failed to find APP_DESCRIPTOR")
offset += 8
...
print("Starting offset:", offset-8, "  hex:", hex(offset-8))
print("crc1:", hex(crc1))
print("crc2:", hex(crc2))
print("len(img):", len(img), "  hex:", hex(len(img)))
print("githash:", hex(githash))

desc = struct.pack('<IIII', crc1, crc2, len(img), githash)
print("desc", desc, "sizeof", sys.getsizeof(desc))

desc_unpacked = struct.unpack('<IIII', desc)
print("desc_unpacked:", desc_unpacked)

img = img[:offset] + desc + img[offset+desc_len:]

''' open("C:/Users/geoma/Desktop/test/UAVCAN/UAVCAN_NODE_FreeRTOS/build/UAVCAN_NODE_FreeRTOS_magic.bin", 'wb').write(img) '''
filepath = Path(__file__).parent / "../build/UAVCAN_NODE_FreeRTOS_magic.bin"
print(filepath)
